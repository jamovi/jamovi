...
def _launch_electron(port_no):
    if os.name == 'nt':
        dot = os.path.abspath(os.path.dirname(os.path.realpath(__file__)))
        dotdot = os.path.abspath(os.path.join(dot, '..'))
        exe = os.path.join(dotdot, 'node_modules', 'electron-prebuilt', 'dist', 'electron.exe')
        main = os.path.join(dotdot, 'electron', 'main.js')
        subprocess.Popen([exe, main, str(port_no)], close_fds=True)
    else:
        subprocess.Popen(['electron', 'electron/main.js', str(port_no)], close_fds=True)    
