...
class Instance:
    ...
    def open(self, request):
        ...
        response.open.status = coms.Status.COMPLETE
    ...
    def _openCallback(self, task, progress):
        response = coms.Response()
        response.open.status = coms.Status.IN_PROGRESS
    ...
    def info(self, id):
        ...
        if hasDataSet:
            ...
            for column in self._dataset:
                ...
                field = coms.InfoResParams.Schema.Field()
                field.name = column.name()
                field.measureType = coms.InfoResParams.Schema.Field.MeasureType(column.column_type)
                field.width = 100
    ...
    def cells(self, request):
        ...
        for c in range(colStart, colStart + colCount):
            ...
            colRes = coms.CellsResParams.Column()
