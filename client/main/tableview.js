...
const TableView = SilkyView.extend({
    ...
    initialize(options) {
        ...
        this.$el.on('transitionstart', event => {
    ...
    onTransitioning: function(layoutKey) {
        ...
        if (layoutKey || ! this.transitionCheckActive) {
            if ( ! layoutKey) {
                ...
                layoutKey = this.$el[0].clientWidth + ' ' + this.$el[0].clientHeight;
            }
            setTimeout(() => {
                let nextLayoutKey = this.$el[0].clientWidth + ' ' + this.$el[0].clientHeight;
                if (layoutKey !== nextLayoutKey) {
                    this.transitionCheckActive = true;
                    this.onTransitioning(nextLayoutKey);
                }
                else if (this.transitionCheckActive){
                    this.transitionCheckActive = false;
                    this.onTransitioning(nextLayoutKey); //one last check
                }
                else {
                    if (this.$el[0].clientHeight === 0)
